...
export default function GalleryExperience({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      const verticalDragFactor = darkBg ? 0.58 : 0.95;
      const verticalRange = darkBg ? viewportLayout.stepY * 2.4 : bounds.height / 2;
      const verticalOffset = clamp(
        offsetRef.current.y * verticalDragFactor,
        -verticalRange,
        verticalRange
      );
      ...
      nodes.forEach((node, index) => {
        ...
        const wrappedY = centeredBaseY + verticalOffset;
  ...
  const cta = ctaHref ? (
    <Link href={ctaHref} className={viewportLayout.ctaClass}>
      {ctaLabel}
    </Link>
  ) : (
    <button
      ...
      className={viewportLayout.ctaClass}
    >
      {ctaLabel}
    </button>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div ref={stageRef} className={viewportLayout.stageClass}>
          {items.map((item) => (
            <img
              key={item.id}
              data-grid-item
              ...
              <div className="mx-auto flex w-full max-w-[540px] flex-col">
                ...
                <form className="mt-10 flex flex-col sm:mt-14" onSubmit={handleSubscribe}>
                  ...
                  <label className="mt-6 flex items-start gap-3 text-[12px] leading-6 text-black sm:mt-7 sm:gap-4 sm:text-[14px] sm:leading-7">
                    ...
                    />
                    <span>
                      I agree to receive marketing emails from Natalie Zytnicka Brooks.
